...
class Question extends Component {
  checkboxGrid = category => {
    const foodArr = this.props.allFood.filter(
      food => food.category === this.props.questionCategory
    );
      ...
      return (
        <React.Fragment>
          <Grid item xs={12} sm={6}>
            <FormGroup>{this.checkboxes(firstFoodArr)}</FormGroup>
          </Grid>
          <Grid item xs={12} sm={6}>
            <FormGroup>{this.checkboxes(secondFoodArr)}</FormGroup>
          </Grid>
        </React.Fragment>
      );
    } else {
  ...
  checkboxes = arr => {
    return arr.map((food, index) => (
      <FormControlLabel
        key={food.id}
        label={
          food.examples ? `${food.name} (${food.examples})` : `${food.name}`
        }
        control={
          <Checkbox
    ...
    return (
      <div>
        <Grid className={classes.formTop}>
          <h2>{this.props.questionCategory}:</h2>
        </Grid>

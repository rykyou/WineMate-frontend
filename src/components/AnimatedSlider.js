...
class AnimatedSlider extends Component {
  content = [
    {
      ...
      description:
        "Don't know where to start with the wine selection? Let us know what's on the menu, and we'll help you find the perfect wine style.",
    ...
    },
    {
      ...
      description:
        "Click to discover the theory behind food and wine pairing and how we find the perfect suggestion based on the flavor profile of your meal.",
      button: "Discover",
      image: BackgroundImage2,
      handleButtonClick: "Open InfographicDialog"
    },
    {
      title: "9 Primary Styles of Wine",
      description:
  ...
  buttonToRender = item => {
    if (item.handleButtonClick === "Go to Quiz") {
      ...
      return (
        <button onClick={this.props.handleClickDialog}>{item.button}</button>
      );
    } else if (item.handleButtonClick === "Go to WineStyles") {
      return (
        <Link to={`/winestyles/${this.props.wineStyleToGoTo}`}>
          <button>{item.button}</button>
        </Link>
      );
    }
